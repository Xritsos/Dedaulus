...
integer::i,j,k,jj,grid_update,dim1,dim2,counter,r_local,theta_local,phi_local
real*8,dimension(:),allocatable::alts
real*8,intent(inout)::m(:)
real::t1,t2
! OpenMP Declarations
integer:: thread_used(0:9),nThreads,iThread
! ==============================================================================
...
! SET UP OpenMP
!$OMP PARALLEL DO                   &
!$OMP  SHARED  (thread_used,counter,deltaphi,deltatheta,m,ne,daed_alt),&
!$OMP& SHARED (daed_lat,daed_lon,grid_lat,grid_lon,grid_lev,zg)   &
!$OMP  PRIVATE (i,iThread,deltarho,alts,w1,w2,w3,w4,w5,w6,w7,w8,dx,dy,dz,theta_local,r_local,phi_local,lat_temp)
...
! Check for outside of grid
  ...
  ! ! Relative distances for interpolation weights
  ! dx=abs((daed_lat(i)-grid_lat(theta_local))/deltatheta)    !elevation direction
  ! dy=abs((daed_lon(i)-grid_lon(phi_local))/deltaphi)        !azimuthal direction
  ! deltarho=alts(r_local+1)-alts(r_local)
  ! dz=abs((daed_alt(i)-alts(r_local))/deltarho)              !azimuthal direction
